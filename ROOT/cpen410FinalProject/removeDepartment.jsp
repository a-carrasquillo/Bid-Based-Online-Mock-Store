		...
		try {
			...
			if(appDBAuth.verifyUserPageAccess(userName, currentPage)) {
				...
				if(res.next()) {
					...
					session.setAttribute("currentPage", "removeDepartment.jsp");
					...
					// Create an instance of the class where the required API method is located
					...
					if(!department.isEmpty()) {
						...
						// that the department was remove successfully
...
<%
						...
						if(appDBMnger.makeDepartmentInactive(department)) {
							// made inactive successfully
%>
							<h1>Department made inactive successfully, redirecting to List of Departments...</h1>
<%
						} else {
%>
							<h1 id="error">Department can not be made inactive at the moment. If the problem persists contact the developers. Redirecting to List of Departments...</h1>
<%
						}
%>
							</body>
